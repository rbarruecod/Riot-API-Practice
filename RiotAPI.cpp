...
void RiotAPI::loadChampionData()
{
    ...
    if (r_champions.status_code != 200)
        return; // Fallo al cargar
                ...
                if (info.contains("participants"))
                {
                    for (const auto &participantJson : info["participants"])
                    {
                        if (participantJson.value("puuid", "") == playerPuuid)
                        {
                            ...
                            // --- Multi-kill data ---
                            summary.kills = participantJson.value("kills", 0);
                            summary.deaths = participantJson.value("deaths", 0);
                            summary.doubleKills = participantJson.value("doubleKills", 0);
                            summary.tripleKills = participantJson.value("tripleKills", 0);
                            summary.quadraKills = participantJson.value("quadraKills", 0);
                            summary.pentaKills = participantJson.value("pentaKills", 0);
                            summary.largestKillingSpree = participantJson.value("largestKillingSpree", 0);
